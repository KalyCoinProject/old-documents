## Raw KRC20 Transaction implementation guide

##### Introduction

The intended audience for this document is developers who need to build transactions that interact with KRC20 tokens. This document consists of two parts. The first part describes how to execute KRC20 methods and watching for events by using various RPC calls on a fully online kalycoind node. The second part describes how to create KRC20 transactions without using kalycoind but rather using an example Bitcoin library to achieve the same functionality. The document assumes the reader is familiar with raw Bitcoin transactions as well as the ERC20 standard and the ABI of Ethereum.



##### Kalycoin's KRC20 standard

Kalycoin's KRC20 standard is virtually identical to Ethereum's ERC20 standard. A KRC20 token contract in Kalycoin typically would use the same template solidity contract as an ERC20 token. The only major exception being that Kalycoin's KRC20 tokens typically use 8 decimal points instead of 18 in Ethereum. 



##### Calldata for KRC20 calls
...
To check the current token balance of an address for a KRC20 contract we need to execute the **balanceOf** method of the KRC20 contract. This execution should be performed as a read-only operation, which means we should use the **callcontract** RPC. An example is displayed below of how to check the balance of a particular address. The output is in hex, padded to 32 bytes, and should be divided by the number of **decimals** used in the contract.

```
ADDRESS="qauZFnmbNBNuY2ujQateDwzvL6zoxBiY3H"
ADDRESS_HEX=$(kalycoin-cli -regtest gethexaddress $ADDRESS)
CALLDATA="70a08231000000000000000000000000$ADDRESS_HEX"
kalycoin-cli -regtest callcontract $KRC20_CONTRACT $CALLDATA | jq -r ".executionResult.output" | python3 -c "print(int(input(), 16))")
```



##### Checking the constants of a KRC20 contract (decimals, name, standard, totalSupply, symbol)

To fetch the constants for a KRC20 contract, we need to execute the various methods of the KRC20 contract. These executions should be performed as read-only operations, which means we should use the **callcontract** RPC. An example is displayed below of how to check the various constants of a KRC20 contract.

```
KRC20_DECIMALS=$(kalycoin-cli -regtest callcontract $KRC20_CONTRACT 313ce567 | jq -r ".executionResult.output" | python3 -c "print(int(input(), 16))")
KRC20_NAME=$(kalycoin-cli -regtest callcontract $KRC20_CONTRACT 06fdde03 | jq -r ".executionResult.output" | xxd -r -p)
KRC20_SYMBOL=$(kalycoin-cli -regtest callcontract $KRC20_CONTRACT 95d89b41 | jq -r ".executionResult.output" | xxd -r -p)
KRC20_TOTALSUPPLY=$(kalycoin-cli -regtest callcontract $KRC20_CONTRACT 18160ddd | jq -r ".executionResult.output" | python3 -c "print(int(input(), 16))")

echo "decimals $KRC20_DECIMALS"
echo "name $KRC20_NAME"
echo "symbol $KRC20_SYMBOL"
echo "totalSupply $KRC20_TOTALSUPPLY"
```



##### Watching for new Transfer events using kalycoind

The example below will watch for new Transfer events emitted by a KRC20 contract indefinitely. The example does this by continuously calling the **waitforlogs** RPC. The **waitforlogs** RPC will block until new events matching the call's arguments are seen. When it has seen events emitted by the contract it returns a list of entries that we can iterate through to fetch the details of one or more events that were emitted. In the example below we watch for all transfer events emitted by a contract. Note: the kalycoind node this runs against must be started with the -logevents argument.

```
# This would be run against a live node listening for new contract events forever
# Note that kalycoind should be started with -logevents to enable event logging
MIN_CONFIRMATIONS=20
LAST_BLOCK=$(kalycoin-cli -regtest getblockcount)
TOPIC="ddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
while true
do
    RESULTS=$(kalycoin-cli -regtest waitforlogs $LAST_BLOCK null "{\"addresses\": [\"$KRC20_CONTRACT\"], \"topics\": [\"$TOPIC\"]}" $MIN_CONFIRMATIONS)
    ...
    do
        ...
        SENDER_ADDRESS=$(kalycoin-cli -regtest fromhexaddress ${SENDER_HEX:24:40})
        RECEIVER_ADDRESS=$(kalycoin-cli -regtest fromhexaddress ${RECEIVER_HEX:24:40})
...
##### Example of KRC20 transfer using an online kalycoind node for signing

The easiest way to do a transfer between different addresses for a KRC20 contract is to simply use the **sendtocontract** RPC. 

```
KRC20_SENDER="qauZFnmbNBNuY2ujQateDwzvL6zoxBiY3H"
KRC20_RECEIVER="qL1ah15xwmxLL75TBxfwiXpoovn6dKV72h"
KRC20_TOKEN_TRANSFER_VALUE=1.1
KRC20_DECIMALS=8

# Adjust the tokens to transfer by the decimals and store the result in hex padded to 32 bytes
KRC20_TOKEN_TRANSFER_VALUE_PADDED_HEX=$(python3 -c "print(hex(int(${KRC20_TOKEN_TRANSFER_VALUE} * 10**0x${KRC20_DECIMALS}))[2:].zfill(64))")

# Convert the base58 receiver address into its 20-byte hex-encoded equivalents
KRC20_RECEIVER_HEX=$(kalycoin-cli -regtest gethexaddress $KRC20_RECEIVER)

# Pad the receiver hex to 32 bytes
KRC20_RECEIVER_PADDED_HEX="000000000000000000000000${KRC20_RECEIVER_HEX}"

# The function hash of "transfer(address,uint256)" concatenated with the receiver address and number of tokens to transfer
CALLDATA="a9059cbb${KRC20_RECEIVER_PADDED_HEX}${KRC20_TOKEN_TRANSFER_VALUE_PADDED_HEX}"

# Create, sign and broadcast the transfer
kalycoin-cli -regtest sendtocontract $KRC20_CONTRACT $CALLDATA 0 100000 0.0000004 $KRC20_SENDER
```



#### In depth on Kalycoin transactions

Kalycoin uses Bitcoin-style transactions. The transaction format is basically identical to Bitcoin transactions with a few modifications to the Bitcoin scripting engine. Kalycoin introduces 4 new opcodes in the scripting engine to allow for interaction with the Kalycoin EVM. The only relevant opcode for this document is OP_CALL (0xc2). 

##### OP_CALL

The OP_CALL opcode allows transaction output scriptPubKeys to be used to execute smart contracts in the Kalycoin EVM. The format for an OP_CALL scripts is the following:
...
The script must follow the same rules for encoding as a typical bitcoin script, e.g., each value that is not an opcode will be prefixed with a push length unless it is a "smallint" (typically any bitcoin library used for creating a transaction will handle these details). Below is a transaction that contains an output that uses an OP_CALL scriptPubKey to execute a contract in Kalycoin.



##### Transaction Signing

The process of signing a raw transaction in Kalycoin is identical to signing a Bitcoin transaction. Typically, a raw transaction is signed using the signrawtransactionwithkey or signrawtransactionwithwallet in kalycoind. Also, any Bitcoin library should be possible to be used to sign raw Kalycoin transactions.



##### Example of a raw KRC20 transfer using a Bitcoin library

Described below is the process of creating and signing a raw KRC20 transfer transaction using the bitcoin-js library.
...
/*
	Example of transaction that creates and signs a KRC20 transfer transaction for Kalycoin using the bitcoin-js library.
	The transaction format is nearly identical to that of Bitcoin, except for the OP_CALL output.
	Note, since OP_CALL is a Kalycoin specific opcode, it needs to be hardcoded as 0xc2
...
const TXINVOUT = 0;
// The WIF key of the input key. Kalycoin uses the same WIF format and prefx as Bitcoin
const TXINKEY = bitcoin.ECPair.fromWIF('cVNrcCHX5q318dDkpMFoLD1ack7TybxEjFB44xCsv5sW35kM7QuE', testnet);
// The value of the utxo, Kalycoin uses the same number of decimals as Bitcoin (8 decimals)
const TXINVALUE = 2000000000000;

// The contract address we wish to call
const KRC20_CONTRACT = "a20ee8612b8d338c55dcd03e65544339efd7cebc";

// The receiver address. Note that the SENDER is the address of the UTXO we are spending.
const KRC20_RECEIVER="qauZFnmbNBNuY2ujQateDwzvL6zoxBiY3H";

// The number of tokens we want to transfer
const KRC20_TOKEN_TRANSFER_VALUE=1n;

// The number of decimals defined by the KRC20 contract.
const KRC20_DECIMALS=8n;

// The minimum gas price is 0.00000040 Kalycoin, so we set the gas price to that here
const GASPRICE = 40;
// The gas schedule of Kalycoin is virtually identical to that of Ethereum, so a value that works for Ethereum will typically work for Kalycoin
const GASLIMIT = 100000;
// The OP_CALL (0xc2) opcode is Kalycoin specific and must therefore be hardcoded.
const OP_CALL = 0xc2;


var krc20_receiver_hex=bitcoin.address.fromBase58Check(KRC20_RECEIVER)['hash'].toString('hex')
var krc20_token_transfer_value_adjusted=KRC20_TOKEN_TRANSFER_VALUE * (10n**KRC20_DECIMALS);
var krc20_token_transfer_value_adjusted_hex=("0".repeat(64) + krc20_token_transfer_value_adjusted.toString(16)).slice(-64)

// The calldata, should be ABI encoded according the Ethereum ABI specification.
var calldata = "a9059cbb000000000000000000000000"+krc20_receiver_hex+krc20_token_transfer_value_adjusted_hex;
...
var contractCallScript = bitcoin.script.compile([
	...
	new Buffer(KRC20_CONTRACT, "hex"), // the contract's address
