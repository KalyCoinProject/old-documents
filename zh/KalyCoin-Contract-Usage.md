...
Kalycoin是一个基于比特币UTXO模型，权益证明机制(pos)和支持EVM智能合约的区块链项目。它通过创新的账户抽象层(Account Abstraction Layer)实现了比特币与以太坊两大生态的融合。如果想了解更多关于Kalycoin的信息，可以访问我们的官网[www.kalycoin.io](www.kalycoin.io)，欢迎加入我们的社区。

## 基于Kalycoin的智能合约
首先需要说明的是，使用Kalycoin的智能合约接口仍然需要一定的技术储备。本文对智能合约的操作在命令行中使用`kalycoin-cli`，或者在图形钱包`kalycoin-qt`的debug窗口中使用。
为了演示如何简单创建和操作一个智能合约，我们将会使用如下的合约代码：

```
pragma solidity ^0.4.0;
contract KalycoinTest {
   uint storedNumber;
   function KalycoinTest() {
       storedNumber=1;
   }
...
*（提示：如果使用QT钱包中的debug窗口，在以下的命令中都不需要再包含`./kalycoin-cli`）*

首先我们创建合约：

    ./kalycoin-cli createcontract  300000
...
现在需要等待你创建的合约被打包进一个区块，你可以通过如下的方法去确认是否被打包：
	...
	./kalycoin-cli getaccountinfo $CONTRACT
...
因为我们的state还没有发生改变，所以调用命令`callcontract`:
	...
	./kalycoin-cli callcontract $CONTRACT 3450bd6a
...
提示，我们加上了`--lenient`这个选项后，可以不用将参数补齐至256bit。现在我们来调用合约：
    ...
    ./kalycoin-cli sendtocontract $CONTRACT 3fb5c1cb000000000000000000000000000000000000000000000000000000000001e240
...
如下的命令将会向合约中发送10个token：
    ...
    ./kalycoin-cli sendtocontract $CONTRACT d0e30db0 10

同样，从合约中提取代币也非常的简单，调用如下的命令：

    ./kalycoin-cli sendtocontract $CONTRACT 3ccfd60b

你也可以手动指定以什么地址为`sender`调用合约，假设我的一个钱包地址是`qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt `，我可以使用如下的命令：

    ./kalycoin-cli sendtocontract $CONTRACT 3ccfd60b 0 190000 0.0000001 qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt

如果你的地址里没有足够的代币，会提示`Sender address does not have any unspent outputs`，这个时候你需要先向地址内转入一定的代币：

    ./kalycoin-cli sendtoaddress qZbjaE8N18ZU1m7851G7QGhvxKL74SRBTt 0.001
...
* Q: "我觉得我发现了一个Kalycoin的bug"
	
	A: 欢迎报送到这里 [https://github.com/kalycoinproject/kalycoin/issues](https://github.com/kalycoinproject/kalycoin/issues)



# Kalycoin新增的RPC命令

Kalycoin支持所有比特币的RPC命令，并且还新加了如下的命令：

* `createcontract` - 在Kalycoin上创建和发布智能合约，会消耗gas
* `callcontract` - 这是一个与Kalycoin上已经部署好的合约进行交互的接口，所有的计算都是在链下进行，不需要消耗gas
* `sendtocontract` - 这也是一个与Kalycoin上已经部署好的合约进行交互的接口，但是所有的计算都是在链上进行的，并且所有的状态改变都会同步到链上。这个命令可以向合约发送代币，会消耗gas
...
Kalycoin支持所有的比特币命令行参数，另外Kalycoin新增如下的参数：

* `-record-log-opcodes` - 在Kalycoin的数据目录(一般是在~/.kalycoin)下新建一个log文件，名字是`vmExecLogs.json`，所有的EVM LOG操作都会被记录。
